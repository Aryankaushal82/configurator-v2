...
const ViewerControls = () => {
  ...
  const getConfiguratorState = () => {
    return window.configuratorState || {
      state: {
        backgroundSetting: background,
        isFullscreen: isFullscreen,
        showCoordinates: showCoordinates // Add this to expose to other components
      },
      setBackgroundSetting: (setting) => {
        setBackground(setting);
        if (window.configuratorState) {
  ...
  // Toggle coordinates display and axes helper
  const toggleCoordinates = () => {
    const newShowCoordinates = !showCoordinates;
    setShowCoordinates(newShowCoordinates);
    
    // Update global state for other components to access
    if (window.configuratorState) {
      window.configuratorState.state.showCoordinates = newShowCoordinates;
    }
    ...
    // Dispatch custom event for axes helper
    const event = new CustomEvent('toggleAxesHelper', { 
      detail: { visible: newShowCoordinates } 
    });
  ...
  md:left-6 md:top-1/2 md:-translate-y-1/2 md:translate-x-0 md:bottom-auto
  md:bg-white/80 backdrop-blur-sm p-2 rounded-lg shadow-lg
  flex flex-row gap-2
  ...
  {/* Background controls */}
  <div className="border-t border-gray-200 md:pt-4 flex flex-row md:flex-col gap-2 md:gap-4">
    <button 
      className={`flex justify-center items-center w-10 h-10 rounded-full shadow-md hover:bg-gray-100 transition-colors ${state.backgroundSetting === 'light' ? 'bg-blue-100' : 'bg-white'}`}
      onClick={() => configurator.setBackgroundSetting('light')}
      aria-label="Light background"
    ...
    <button 
      className={`flex justify-center items-center w-10 h-10 rounded-full shadow-md hover:bg-gray-100 transition-colors ${state.backgroundSetting === 'dark' ? 'bg-blue-100' : 'bg-white'}`}
      onClick={() => configurator.setBackgroundSetting('dark')}
      aria-label="Dark background"
    >
      <Moon size={20} />
    ...
    <button 
      className={`flex justify-center items-center w-10 h-10 rounded-full shadow-md hover:bg-gray-100 transition-colors ${state.backgroundSetting === 'gradient' ? 'bg-blue-100' : 'bg-white'}`}
      onClick={() => configurator.setBackgroundSetting('gradient')}
      aria-label="Gradient background"
    >
      <Palette size={20} />
